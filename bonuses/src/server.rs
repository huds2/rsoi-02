...
async fn get_handler(username: String,
...
async fn purchase_handler(username: String,
                          body: PurchasePost,
                          privilege_repository: Arc<Mutex<dyn PrivilegeRepository>>) -> WebResult<Box<dyn Reply>> {
...
async fn refund_handler(username: String,
...
pub async fn run_server(repository: Arc<Mutex<dyn PrivilegeRepository>>, port: u16) {

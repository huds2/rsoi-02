...
async fn list_handler(username: String,
                      ticket_repository: Arc<Mutex<dyn TicketRepository>>) -> WebResult<Box<dyn Reply>> {
...
pub fn router(repository: Arc<Mutex<dyn TicketRepository>>) -> impl Filter<Extract = impl Reply, Error = Rejection> + Clone {
...
pub async fn run_server(repository: Arc<Mutex<dyn TicketRepository>>, port: u16) {
